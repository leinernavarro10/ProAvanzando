...
function verificarFE(numeroFactura,sistema){
  ...
  jQuery.post("funcionesAJAXFacturas.php", { f:'VERIFICAR',numeroFactura:numeroFactura,sistema:sistema})
      .done(function(data){
        ...
        if (data.indexOf('<br />') >= 0){
            ...
            $("#estadoFE-"+numeroFactura).html("<center><img src='../assets/images/cargando.gif'> CAMBIAR ESTADO...</center>");
	          jQuery.post( "funcionesAJAXFacturas.php", { f: 'CAMBIARESTADO',sistema:sistema,numeroFactura: numeroFactura, estado: estado, xmlRespuesta: resp['respuesta-xml']})
	          .done(function( data ) {
...
function anular(numeroFactura, idFactura, ori){
  var tipoDoc = $("#tipoDoc").val();
	...
	console.clear();
	$("#botoneraFinalizar").html("<center>Generando XML.</center>");
...
function firmarXML(numeroFactura, idFactura){
  ...
  $("#botoneraFinalizar").html("<center>Firmando archivo XML.</center>");
...
function token(numeroFactura, idFactura){
  $("#botoneraFinalizar").html("<center>Solicitando token de acceso a hacienda.</center>");
...
function enviarFactura(numeroFactura, token, idFactura){
  ...
  $("#botoneraFinalizar").html("<center>Enviando factura a hacienda.</center>");
...
function tokenR(numeroFactura,sistema){
	$("#estadoFE-"+numeroFactura).html('<center><img src="../assets/images/cargando.gif"> Token...</center>');
	jQuery.post( "funcionesAJAXFacturas.php", { f: 'OBTENER-TOKEN',sistema:sistema})
      .done(function( token ) {
      	if (token.indexOf('<br />') >= 0){
            $("#estadoFE-"+numeroFactura).html('<center><img src="../assets/images/cargando.gif"> CAMBIAR ESTADO...</center>');
      		jQuery.post( "funcionesAJAXFacturas.php", { f: 'CAMBIARESTADO', numeroFactura: numeroFactura, estado: 4, xmlRespuesta: "",sistema:sistema})
	          .done(function( data ) {
  	  		...
  	  		enviarFacturaR(numeroFactura, token,sistema);
  	    }
      	//console.log(token);
    });
}
...
function enviarFacturaR(numeroFactura, token,sistema){
	ids = '<?php echo $idFactura; ?>-';
  var emailNC = $("#email"+numeroFactura).val();
  if(emailNC!=""){
    var idFactura = $("#idFactura"+numeroFactura).val();
	...
	$("#estadoFE-"+numeroFactura).html('<center><img src="../assets/images/cargando.gif"> Enviando...</center>');
	jQuery.post( "funcionesAJAXFacturas.php", { f: 'ENVIAR-FACTURA', numeroFactura: numeroFactura, token: token,sistema:sistema})
      .done(function( data ) {
...
//	          $("#estadoFE-"+numeroFactura).html('No enviada');
          $("#estadoFE-"+numeroFactura).html('<center><img src="../assets/images/cargando.gif"> CAMBIAR ESTADO...</center>');
	          ...
	          jQuery.post( "funcionesAJAXFacturas.php", { f: 'CAMBIARESTADO', numeroFactura: numeroFactura, estado: estado, xmlRespuesta: resp['respuesta-xml'],sistema:sistema})
	          .done(function( data ) {
