...
var neonChat = neonChat || {
	...
	chat_history: [],
	statuses: {
		online: {class: 'is-online', order: 1, label: 'Red msg'},
		offline: {class: 'is-offline', order: 4, label: 'Offline'},
		idle: {class: 'is-idle', order: 3, label: 'Idle'},
		busy: {class: 'is-busy', order: 2, label: 'Online'}
	}
};
...
;(function($, window, undefined)
{
	...
	$.extend(neonChat, {
		...
		open: function($user_link)
		{
			...
			$.post('../chat/verchat.php', {id: id}, function(data) {
				data=data.split("||");
			this.pushMessage(id,data[0], $chat.data('current-user'), new Date()) ;
		...
		submitMessage: function() // Submit whats on textarea
		{
			...
			if(this.isOpen && this.$current_user)
			{
				...
				if(msg!=""){
					$.post('../chat/guardar.php', {msg: msg, id:id}, function(data) {
						if(data!="1"){
							msg=data;
						}
